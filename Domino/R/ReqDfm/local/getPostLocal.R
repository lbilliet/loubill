...
nomFichier<-"alt"
chemin<-paste("data/",nomFichier,".csv")
df <- scan(file = chemin, sep="\t", what = character(), quote = "")
df <- scan(file = data, sep="\t", what = character(), quote = "")

getConcatenation = function(chemin){
  df <- scan(file = chemin, sep="\t", what = character(), quote = "")
  df <- unique(df)
  concatenation <- paste(df$contentPost,collapse="\n")
  matrice <- quanteda::dfm(concatenation, remove = stopwords.fr, remove_punct=TRUE, remove_numbers = TRUE, tolower = TRUE, verbose = TRUE)
